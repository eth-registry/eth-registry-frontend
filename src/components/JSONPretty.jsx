...
module.exports = createReactClass({
  ...
  _replace: function(match, ind, key, val, tra) {
    ...
    if (val) {
      if (val === "true" || val === "false") {
        ...
        let originalVal = val.replace(/"/gi, "");
        if (key) {
          if (key.indexOf("abi") > -1 || key.indexOf("source") > -1) {
            val =
              '"' +
              "<a href='https://ipfs.infura.io/ipfs/" +
              originalVal +
              "'>" +
              originalVal +
              "</a>" +
            ...
            val = "<img src='" + originalVal + "' class='icon'/>";
          } else {
            if (val.length > 44) {
              val = val.substr(0, 44) + '..."';
            }
  ...
  _pretty: function(obj, replacer, space) {
    ...
    var regLine = /^( *)("[^"]+": )?("[^"]*"|[\w.+-]*)?([,[{]|\[\s*\],?|\{\s*\},?)?$/gm;
    var text = JSON.stringify(
      obj,
      typeof replacer === "function" ? replacer : null,
      isNaN(space) ? 2 : space,
    );
  ...
  render: function() {
    ...
    var classNameFinal = className
      ? className + " " + themeClassNameFinal
      : themeClassNameFinal;
    ...
    if (typeof json === "string") {
      try {
        ...
        return (
          <pre
            {...rest}
            className={classNameFinal || "json-pretty"}
    ...
    return (
      <pre
        ...
        dangerouslySetInnerHTML={{
          __html: this._pretty(json, replacer, +space),
        }}
      />
    );
  },
